...
bool roper_server::send_vrpn_image(vrpn_Imager_Server* svr,vrpn_Connection* svrcon,double g_exposure,int svrchan, int num_chans)
{
    ...
    for(y=0;y<num_y;y=min(num_y,y+nRowsPerRegion)) {
      svr->send_region_using_base_pointer(svrchan,0,num_x-1,y,min(num_y,y+nRowsPerRegion)-1, reinterpret_cast<vrpn_uint16 *>(_memory) + offset, stride, num_x * stride);
      svr->mainloop();
    }
...
bool roper_server::write_to_opengl_texture(GLuint tex_id)
{
  ...
  const void*   SUBSET_BUFFER = &((uns16 *)_memory)[NUM_COMPONENTS * ( _minX + get_num_columns()*_minY )];
...
bool spe_file_server::write_opengl_texture_to_quad()
{
