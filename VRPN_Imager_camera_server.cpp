...
// XXX This routine needs to be tested.
bool VRPN_Imager_camera_server::send_vrpn_image(vrpn_Imager_Server* svr,vrpn_Connection* svrcon,double g_exposure,int svrchan, int) const
{
...
bool VRPN_Imager_camera_server::write_to_opengl_texture(GLuint tex_id)
{
  ...
  if (!_opengl_texture_have_written) {
    ...
    if (tempimage == NULL) {
      fprintf(stderr,"VRPN_Imager_camera_server::write_to_opengl_texture(): Out of memory allocating temporary buffer\n");
      return false;
    }
  ...
  glTexSubImage2D(GL_TEXTURE_2D, 0,
    ...
    FORMAT, TYPE, &((vrpn_uint16 *)_front)[NUM_COMPONENTS * ( _minX + get_num_columns()*_minY )]);
  return true;
}
